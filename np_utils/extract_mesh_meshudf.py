...
def get_udf_normals_grid_slow(func, func_grad, N=56, max_batch=int(2 ** 20), voxel_origin=None, cube_size=2.0):
    """
    ...
    voxel_origin = [-1, -1, -1] if voxel_origin is None else voxel_origin
...
def get_udf_normals_grid_fast(func, func_grad, samples, indices, N=56, max_batch=int(2 ** 18), voxel_origin=None, cube_size=None):
    """
    ...
    if indices is None or samples is None:
        return get_udf_normals_grid_slow(func, func_grad, N, max_batch, voxel_origin=voxel_origin, cube_size=cube_size)
    # network.eval()
    ################
...
def get_mesh_udf_fast(func, func_grad, samples=None, indices=None, N_MC=128,
                      ...
                      voxel_origin=None, cube_size=2.0):
    """
    ...
    df_values, normals, samples = get_udf_normals_grid_fast(func, func_grad, samples, indices, N=N_MC,
                                                            voxel_origin=voxel_origin, cube_size=cube_size)
    ...
    verts = verts + voxel_origin  # since voxel_origin = [-1, -1, -1]
